...
### 32. Documenting a "just a topic name" value as plaintext in a public, recruiter-linked repo
**Cause:** the Enterprise GitOps upgrade below originally printed the real ntfy.sh topic name directly in this file, reasoning that an obscure random string wasn't really a "secret" in the Kubernetes-Secret sense. That reasoning missed the actual threat model: this repo is public and linked from LinkedIn (see `docs/00-for-recruiters.md`), and an ntfy.sh topic has no authentication beyond its name being unguessable - writing it here made it exactly as guessable as "publicly readable by anyone."
**Fix:** rotated to a brand-new topic, resealed the `ntfy-alertmanager-config` SealedSecret with it (ciphertext only, committed as normal), and removed the plaintext value from this file entirely - replaced with a pointer to BookStack (a private wiki, not this repo) as the one place the real value lives now. The retired topic is dead; anyone finding it in older git history gains nothing.
**Lesson for next time:** "is this a Kubernetes Secret" and "is this safe to write in plaintext in this specific repo" are different questions - this repo is public and recruiter-facing (see `docs/00-for-recruiters.md`), so anything that grants access or capability on its own (API topic names, webhook URLs with embedded tokens, etc.) needs the same treatment as a password, even when no formal "Secret" object is involved. When in doubt, default to a placeholder in git plus the real value in BookStack, not the other way around.

### 33. App-of-Apps means pausing one child Application's auto-sync isn't enough
**Cause:** tried to test whether a real Tandoor outage actually triggers an alert, by patching the live `tandoor` Application to remove `syncPolicy.automated` (so a manual `kubectl scale --replicas=0` wouldn't get instantly reverted), then scaling down. Tandoor came back on its own within seconds instead of staying down. Root cause: `root-app` (the App-of-Apps root from the Enterprise GitOps upgrade below) manages the live `tandoor` Application object as one of *its own* resources, with its own `selfHeal: true`. The moment the patch was applied, root-app's own selfHeal saw it as drift from git and reverted it - via a live-object watch, not the ~3 minute git-poll interval, so this happened in well under a minute - which restored `tandoor`'s selfHeal, which then immediately saw `replicas: 0` as drift from git's `replicas: 1` and scaled it back up. Two independent selfHeal layers, each individually correct, compounding into "pausing an Application no longer actually pauses it."
**Fix:** none applied generally - for that specific test, the practical workaround was to stop relying on pausing sync at all and instead build real tooling for it (see the fast-alert + auto-silence-hook work below). If you do need to manually pause a child Application's sync going forward, you must pause **both** `root-app` and the child Application, in that order, or root-app will keep reverting your pause on the child before you get to use it.
**Lesson for next time:** App-of-Apps changes what "pause this Application's sync" means - it's no longer a single edit. Verify a pause actually held (watch `metadata.generation` or just re-`kubectl get` the field a few seconds later) before trusting it, the same lesson as bug 30.

### 34. Alertmanager silently dropped every alert since Epic 4, discovered only by testing a real outage
**Cause:** `kube-prometheus-stack.yaml`'s custom `alertmanager.config` (from the Enterprise GitOps upgrade below) defined a `receivers:` list containing only `ntfy`. The chart itself auto-injects a `route.routes[]` entry sending the `Watchdog` alert to a receiver literally named `"null"` (its convention for "silence this"), regardless of what `receivers:` a user supplies. Because that receiver was never actually defined, the Prometheus Operator's *own* validation step rejected the entire config outright: `provision alertmanager configuration: failed to initialize from secret: undefined receiver "null" used in route`. On rejection, the Operator does not clear or block the currently-running config - it silently keeps serving the last-good one, which in this case was the untouched default (also nothing configured, every alert routed nowhere). Helm rendered the Secret correctly, ArgoCD reported `Synced`/`Healthy` throughout, and even `kubectl get secret alertmanager-kube-prometheus-stack-alertmanager` (the *input* secret Helm renders) showed the intended config - nothing in the normal GitOps status chain ever surfaced the failure.
**What actually surfaced it:** testing whether a real (not synthetic) Tandoor outage produces a real notification (see bug 33) exposed that it didn't - and investigating why led to discovering that *no* real Prometheus-generated alert had ever reached ntfy, including several that had been firing continuously for hours (`KubeControllerManagerDown`, `KubeSchedulerDown`, `KubeProxyDown`). The Enterprise GitOps upgrade's "verified end-to-end" claim for Alertmanager → ntfy was true only for the bridge itself - that test POSTed a synthetic payload directly to the `ntfy-alertmanager` bridge's HTTP endpoint, bypassing Alertmanager (and this bug) entirely. Real evidence lives in exactly two places: `kubectl logs -n monitoring deploy/kube-prometheus-stack-operator` (shows the rejection explicitly), and `kubectl get secret alertmanager-kube-prometheus-stack-alertmanager-generated` (the Operator's actual output secret, mounted by the Alertmanager pod - note the `-generated` suffix; the similarly-named secret without it is just Helm's input and was never the problem).
**Fix:** added `- name: 'null'` to the `receivers:` list. Verified by re-checking the `-generated` secret (now shows `route.receiver: ntfy`) and confirming the already-firing real alerts actually arrived on ntfy and later resolved cleanly.
**Lesson for next time:** for kube-prometheus-stack's `alertmanager.config`, `kubectl get application <name> -o Synced/Healthy` proves the Secret was *applied*, not that Alertmanager is actually *running* it - the Prometheus Operator has its own semantic validation one layer further in that none of the usual GitOps status fields reflect. Verify a custom Alertmanager config change by making a real alert fire (or checking the `-generated` secret directly), not by checking sync status.

### 35. `NodeClockNotSynchronising` firing on all three nodes simultaneously - not an NTP problem
**Cause:** all three K3s nodes fired `NodeClockNotSynchronising` at once (a real notification, not a drill this time - the fast alerting pipeline built earlier the same day actually caught something unplanned). The obvious suspects - NTP blocked by the firewall, or the router's NTP server down - were both ruled out directly: UDP 123 to both `pool.ntp.org` and the router (`192.168.178.1`) worked fine from every node, and `timedatectl show-timesync` showed a valid NTP response had been received hours earlier. The actual cause: `systemd-timesyncd` restarts its entire connection attempt on every "network configuration changed" event it sees, and a K3s node generates a lot of those - every pod create/destroy adds or removes a veth interface, which is exactly the kind of event timesyncd treats as a topology change worth restarting for (`journalctl -u systemd-timesyncd` showed dozens of "Network configuration changed, trying to establish connection" lines in the space of a few minutes, with no successful sync in between). On a node with enough pod churn, timesyncd can end up permanently restarting before it ever completes a fresh handshake - a known weakness of timesyncd's simplistic design on container hosts specifically, not a homelab-specific fluke.
**Fix:** `ansible/playbooks/fix-time-sync.yml` - installs chrony (which tolerates this kind of interface churn without restarting its whole state machine) on every node via the `k3s_cluster` group. One live surprise: installing chrony via `apt` already removed the `systemd-timesyncd` unit entirely as a package-conflict side effect (its dbus alias was left behind in a harmless "bad" state) - the playbook's own explicit "stop and disable systemd-timesyncd" task then failed outright (`Could not find the requested service`) because the unit was already gone by the time it ran. Made idempotent by checking `systemctl list-unit-files systemd-timesyncd.service` first and skipping the stop/disable task if it's already absent - handles both a fresh node (timesyncd present, needs stopping) and this case (chrony already removed it).
**Verification:** `timedatectl show --property=NTPSynchronized --value` returns `yes` on all three nodes (the playbook's last task retries until this is true, so a normal run already proves it), and the three `NodeClockNotSynchronising` alerts confirmed cleared from Alertmanager's active list afterward - not assumed from the playbook reporting success.
**Lesson for next time:** the same alert firing identically on every node at once is itself a diagnostic clue - it points at something systemic (a shared package/config/behavior) rather than one bad host, and is worth checking for a common cause before investigating each node individually.

### 36. `TargetDown` for `longhorn-backend` - port 9500 is bound but every connection is refused, cause unresolved
**Cause:** genuinely not fully root-caused, documented honestly rather than papered over. All three K3s nodes rebooted together around 08:04 UTC on 2026-08-29 (visible in every node's process start times and in all three `longhorn-manager` pods restarting within 20 seconds of each other). Since then, Prometheus has been unable to scrape `http://<pod-ip>:9500/metrics` on any of the three `longhorn-manager` pods - `dial tcp ...: connect: connection refused`, confirmed both from Prometheus's own live target status and from a manually-run debug pod. What makes this stranger than a normal "pod isn't up" case: `ss -tlnp` run *inside* the same `longhorn-manager` container clearly shows a `LISTEN` socket bound to that exact pod IP on port 9500, at the same moment an external connection to that identical address is refused. Deleting all three pods (fresh pods, fresh IPs, fresh listeners) reproduced the exact same symptom within seconds of the new pods starting - so this isn't leftover stale state from the reboot, it's a live, reproducible condition.
**What this is not:** a data-safety problem. `kubectl get volumes.longhorn.io -A` shows every attached volume `healthy`, and all three Longhorn nodes are `Ready`/`Schedulable`. This is isolated to the Prometheus metrics scrape path specifically - Longhorn's actual storage/replication function was never in question, confirmed directly rather than assumed.
**Investigation dead ends, for whoever picks this up next:** not a firewall/NetworkPolicy issue (this cluster has none, and the failure reproduces for same-node connections too, not just cross-node). Not the classic "port simply never listens" failure mode from [longhorn/longhorn#10004](https://github.com/longhorn/longhorn/issues/10004) (a similar-sounding but distinct report against v1.6.3) - in that issue `ss` shows no listener at all; here it clearly does. Most likely explanation not yet confirmed: `longhorn-manager` may be rapidly closing and rebinding its metrics listener as part of some internal reconciliation loop (the pod logs show heavy controller cache-sync activity continuing well after startup), creating a listener that's real but not actually accepting connections during most of its lifetime - consistent with `ss` catching it in a bound-but-not-serving window.
**Not attempted:** upgrading the Longhorn chart/app version to see if a newer release fixes this. Longhorn here is installed out-of-band (Helm, not GitOps - see bug 26) and there is still no backup target configured (NAS not yet available as of this writing) - upgrading the storage layer itself with no safety net is a deliberate, bigger decision, not something to do as a side effect of chasing a warning-severity metrics gap.
**Current mitigation:** none beyond restarting the pods, which did not help. Left as an open, honestly-documented gap - see "Known limitations" below.
...
4. **Alertmanager → ntfy.sh** (`kubernetes/apps/ntfy-alertmanager.yaml`, `kube-prometheus-stack.yaml`'s new `alertmanager.config`) - Alertmanager's `webhook_configs` has no templating and can't reshape its fixed JSON schema into ntfy's title/priority/tags fields, so a small bridge (`xenrox/ntfy-alertmanager`, scfg-configured) sits in between. The bridge's config file holds the ntfy.sh topic name - a bearer credential on a public server, and this repo is public - so it's a `SealedSecret`, not a plain `ConfigMap` (a deviation from this epic's original ask, which assumed a Helm values change alone would be enough).
   ...
   **ntfy.sh topic:** deliberately **not written here**. This repo is public (linked from LinkedIn for recruiters) - an ntfy.sh topic name has no authentication beyond obscurity, so it's a bearer credential, not just a label. The real topic name lives in BookStack (`Setup Overview` page - a private wiki, not this public repo) and nowhere else in git. An earlier revision of this section *did* commit the actual topic in plaintext; that topic has since been retired and resealed under a new one specifically because of that exposure - if you're reading this from git history, that old topic is dead, don't use it.

   Verified end-to-end with a synthetic `firing` webhook POSTed directly to the bridge from inside the cluster - it arrived at ntfy.sh with the correct title, `priority: 3`, and `tags: ["warning"]` for a `severity: warning` label. **Correction (same day, see bug 34):** that test only proved the bridge itself works - it POSTed straight to the bridge's HTTP endpoint, never through Alertmanager, so it didn't catch that Alertmanager was silently rejecting the whole receiver config and had never actually routed a single real alert anywhere. Fixed a few hours later the same day.

---

## Alerting pipeline hardening (2026-08-29, later the same day)

Prompted by asking "does a real container going down actually notify me" and testing it for real instead of assuming the earlier synthetic test covered it (see bugs 33-34). Once the null-receiver bug was fixed and real alerts started actually reaching ntfy for the first time, several more gaps became visible immediately:

1. **Fast alerting for single-replica apps** (`observability-extras/critical-apps-fast-alert.yaml`) - kube-prometheus-stack's default rules need 15 minutes of sustained badness before firing, reasonable for a large fleet, far too slow for a homelab app with zero redundancy. `CriticalAppDown` fires after 1 minute of zero available replicas, scoped to `tandoor` and `bookstack` only (regex-anchored, so it does not match `tandoor-staging`, which is deliberately scaled to 0 most of the time). The cluster-wide 15-minute defaults are untouched, still covering every other app as a slower backstop.
2. **Automatic deploy-window silencing** (`manifests/tandoor/hook-deploy-notify-*.yaml`, `manifests/bookstack/hook-deploy-notify-*.yaml`) - ArgoCD PreSync/PostSync hook Jobs, running automatically on every git-triggered deploy with no manual step. PreSync creates a 10-minute Alertmanager silence for that app's `CriticalAppDown` and posts a `DeploymentInProgress` (severity info) notice; PostSync resolves it. Both scripts swallow every `curl` failure (`|| true`, no `-f` flag, no `set -e`) deliberately - a PreSync hook that fails **blocks the actual sync**, so a missed notification must never be allowed to block a real deploy.
3. **K3s-specific false positives disabled** (`kubeControllerManager`/`kubeScheduler`/`kubeProxy`: `enabled: false` in `kube-prometheus-stack.yaml`) - K3s bundles these into its single binary rather than running them as separately scrapeable components, so the chart's default checks for them can never find a real target. These three had been firing continuously since the stack was first installed; only noticed because fixing bug 34 was the first time they actually reached ntfy instead of silently going nowhere.
4. **Concise notification body** (`manifests/ntfy-alertmanager/notification.tmpl`) - the bridge's default body dumps every label and annotation as a markdown list, unreadable on a phone lock screen. Custom Go `text/template` (mounted from the same `SealedSecret` as the scfg config, since `payload`/`alert` are the bridge's internal Go struct field names, not documented anywhere public - had to read `main.go` on Codeberg to get them right) now renders one line: `down: <namespace>/<alertname> - <summary>`, plus a pointer to a new BookStack page.
5. **BookStack "Alerts" page** (`Homelab Infrastructure` book, page id 4) - what every alert type actually means and what to check first, since the notification body is now deliberately too short to explain that itself.

All of the above verified with real, not synthetic, evidence: forced a real Tandoor sync and confirmed the PreSync/PostSync hooks fired and resolved a real Alertmanager alert; confirmed the three K3s false positives both fired once (proving the routing fix) and then cleanly resolved once disabled; confirmed the new template's actual rendered ntfy message text.

---

## Supply-chain hardening & observability round 2 (2026-08-29, evening)

Prompted by "what's technically still missing here." Six pieces, landed as separate commits, most requiring live iteration to get right rather than working first try:

1. **Secret scanning** (`.github/workflows/gitleaks.yml`) - `gitleaks-action`, free for personal-account repos (only orgs need a license). Directly motivated by bug 32 - would have caught that incident automatically.
2. **Pinned `:latest` tags** - `vabene1111/recipes` → `2.6.13`, `lscr.io/linuxserver/bookstack` → `26.05.3`, in both the production manifests and the `tandoor-staging` overlay's own copy (easy to miss - it's a separate file, not a reference to prod's). Bug 29 was the direct, already-realized cost of leaving these unpinned.
3. **Renovate** (`renovate.json`) - covers what Dependabot structurally cannot for this repo: ArgoCD `targetRevision` (Helm chart versions) and raw image tags in plain manifests, via the `argocd` and `kubernetes` managers. Neither has a default file pattern (confirmed against current Renovate docs, not assumed) - both needed explicit `managerFilePatterns`. Requires installing the Renovate GitHub App once, manually - the one part of this round that isn't fully self-contained.
4. **Kyverno + `disallow-latest-tag`** (`kyverno.yaml`, `kyverno-policies.yaml`) - policy-as-code, `Audit` mode (not `Enforce` - existing workloads and third-party charts haven't been checked against it yet). Confirmed live actually evaluating: `kubectl get clusterpolicy` shows `Ready: True` and it's generated background `PolicyReport`s for every existing pod. Same split-Application-plus-CRD-race pattern as `metallb`/`metallb-config`. Its own CRDs (the newer `policies.kyverno.io` v2 group - `deletingpolicies`, `validatingpolicies`, etc., none of which this deployment actually uses) ended up in the same perpetual-`OutOfSync` state as `bgppeers.metallb.io` (bug 24) even with `ServerSideApply=true` set - cosmetic, Kyverno itself works, but the exact mechanism is still unexplained the same way bug 24 is.
5. **Loki + Alloy** (`loki.yaml`, `alloy.yaml`) - closes the metrics-but-no-logs gap. Took four live-and-fail iterations to get the Loki chart's values right, each a real chart requirement not mentioned anywhere obvious upfront:
   - `loki.storage.type` defaults to `s3` regardless of `deploymentMode`, and then demands `bucketNames` - had to set it to `filesystem` explicitly.
   - `deploymentMode: Monolithic` does **not** zero out the simple-scalable-mode `read`/`write`/`backend` components - they default to non-zero and the chart refuses to render with both "modes" active. Had to set all three to `replicas: 0` explicitly.
   - `schema_config` has no default at all ("individual for every Loki cluster" per the chart's own error) - added the chart's own documented standard (`tsdb` + `filesystem`, schema `v13`).
   - The chart defaults to multi-tenant mode - every query needs an `X-Scope-OrgID` header unless `loki.auth_enabled: false` is set, which it now is (single cluster, single Grafana, no tenant separation needed).
   - Verified for real, not just "pods are Running": queried Loki's HTTP API directly for `{namespace="argocd"}` and got back real ArgoCD controller log lines (one of them, fittingly, was ArgoCD's own log of reconciling the `loki` Application).
6. **Resource requests/limits** on this repo's raw-manifest apps (`tandoor`, `tandoor-postgres`, `bookstack`, `bookstack-db`, `ntfy-alertmanager`) - values set from observed live `kubectl top pod` usage plus headroom, not guessed, specifically to avoid OOMKilling something that was already running fine. Helm-chart-managed apps (Immich, kube-prometheus-stack, cert-manager, MetalLB, Sealed Secrets, Loki, Alloy, Kyverno) were left on their own chart defaults rather than blindly overriding every third party's tuning.

**Deliberately not done in this round:** PodDisruptionBudgets. Every app in this cluster is single-replica - a PDB with `minAvailable: 1` on a 1-replica workload doesn't protect availability (there's no second replica to shift traffic to), it just blocks voluntary node drains outright, which would be a surprise the next time a node needs maintenance. Not adding one isn't an oversight; it's a judgment call that a PDB here would trade a real inconvenience for no real benefit.

**NAS-Backup placeholder** (`longhorn-ui/backup-target.yaml`) - no NAS exists yet (arriving in a few weeks). Updated the prepared value to use a placeholder hostname, `NAS-Backup` (NFS instead of the old S3 example - simpler, more typical for a home NAS), so the only step left once the NAS is racked is swapping the placeholder for its real hostname/IP, doing the one-time manual UI/API touch bug 26 already documents, and letting ArgoCD take over from there.
...
- **`bgppeers.metallb.io`'s perpetual OutOfSync** (bug 24) — cosmetic, not blocking, but still an open item if anyone wants to actually root-cause it rather than accept the current explanation. Kyverno's newer `policies.kyverno.io` v2 CRDs (2026-08-29) landed in the same unexplained state despite `ServerSideApply=true` already being set - same open question, now with a second data point.
...
- **Terraform/Ansible linting is not in CI** — the pipeline covers Kubernetes manifests and secret scanning now; `terraform validate` and `ansible-lint` on push are still unaddressed.
- **Kyverno's `disallow-latest-tag` policy is Audit-only, not enforced** — logs violations (`kubectl get policyreport -A`) without blocking anything. Needs a pass through those reports to confirm nothing currently running (particularly inside third-party charts like Immich) would actually break before flipping it to `Enforce`.
- **ntfy.sh topic has no authentication beyond obscurity** — the topic name is sealed (encrypted-at-rest) everywhere it lives as a Kubernetes manifest, and (after the plaintext-in-a-public-repo mistake described above) is now kept only in BookStack, a private wiki, never in this git repo. Still, anyone who ever learns the topic name can subscribe to or publish fake alerts to it - fine for a homelab (worst case: spam notifications or a missed real alert, not a cluster security breach), but ntfy's access-token auth would be the real fix if this ever matters more than that.
- **No dead-man's-switch for the alerting pipeline itself** — `Watchdog` is silenced by design (see the BookStack Alerts page), and `NtfyAlertmanagerBridgeDown` can't notify if the bridge is down for its *entire* evaluation window. An out-of-band service (e.g. healthchecks.io) pinged periodically would close this properly; not implemented.
- **Fast (1-minute) app-down alerting only covers Tandoor and BookStack** — Immich's multi-Deployment structure (server, machine-learning) needs more thought on what "down" means for it before it gets the same treatment.
- **`longhorn-backend`'s metrics port (9500) refuses every connection despite `ss` showing it listening** (bug 36) — genuinely unresolved, not a firewall or stale-state issue (survives a full pod restart with fresh IPs). Does not affect actual storage health, which is separately confirmed fine. Fixing this properly likely means a Longhorn version upgrade, deliberately not attempted while there's still no backup target configured.
